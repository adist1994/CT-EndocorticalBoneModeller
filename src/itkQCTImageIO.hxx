 ...
 *
 *=========================================================================*/
#ifndef itkQCTImageIO_hxx
#define itkQCTImageIO_hxx
...
namespace itk
{
    ...
    QCTImageIO< TPixel, VImageDimension >::QCTImageIO():
            ImageIOBase()
    {
    ...
    template< typename TPixel, unsigned int VImageDimension >
    void QCTImageIO< TPixel, VImageDimension >::PrintSelf(std::ostream & os, Indent indent) const
    {
    ...
    template< typename TPixel, unsigned int VImageDimension >
    SizeValueType QCTImageIO< TPixel, VImageDimension >::GetHeaderSize()
    {
        std::ifstream file;
    ...
    template< typename TPixel, unsigned int VImageDimension >
    void QCTImageIO< TPixel, VImageDimension >
...
#define itkReadQCTBytesAfterSwappingMacro(StrongType, WeakType)   \
  ( this->GetComponentType() == WeakType )                        \
    ...
    if ( m_ByteOrder == LittleEndian )                            \
      ...
      InternalByteSwapperType::SwapRangeFromSystemToBigEndian(    \
        ...
        if itkReadQCTBytesAfterSwappingMacro(unsigned short, USHORT)
        else if itkReadQCTBytesAfterSwappingMacro(short, SHORT)
        else if itkReadQCTBytesAfterSwappingMacro(char, CHAR)
        else if itkReadQCTBytesAfterSwappingMacro(unsigned char, UCHAR)
        else if itkReadQCTBytesAfterSwappingMacro(unsigned int, UINT)
        else if itkReadQCTBytesAfterSwappingMacro(int, INT)
        else if itkReadQCTBytesAfterSwappingMacro(long, LONG)
        else if itkReadQCTBytesAfterSwappingMacro(unsigned long, ULONG)
        else if itkReadQCTBytesAfterSwappingMacro(float, FLOAT)
        else if itkReadQCTBytesAfterSwappingMacro(double, DOUBLE)
    }

    template< typename TPixel, unsigned int VImageDimension >
    bool QCTImageIO< TPixel, VImageDimension >
...
#define itkWriteQCTBytesAfterSwappingMacro(StrongType, WeakType)        \
  ( this->GetComponentType() == WeakType )                              \
    {                                                                   \
    typedef ByteSwapper< StrongType > InternalByteSwapperType;          \
    ...
    if ( m_ByteOrder == LittleEndian )                                  \
      ...
      file.write(static_cast< const char * >( buffer ), numberOfBytes); \
            ...
            // Swap bytes if necessary
            if itkWriteQCTBytesAfterSwappingMacro(unsigned short, USHORT)
            else if itkWriteQCTBytesAfterSwappingMacro(short, SHORT)
            else if itkWriteQCTBytesAfterSwappingMacro(char, CHAR)
            else if itkWriteQCTBytesAfterSwappingMacro(unsigned char, UCHAR)
            else if itkWriteQCTBytesAfterSwappingMacro(unsigned int, UINT)
            else if itkWriteQCTBytesAfterSwappingMacro(int, INT)
            else if itkWriteQCTBytesAfterSwappingMacro(long, LONG)
            else if itkWriteQCTBytesAfterSwappingMacro(unsigned long, ULONG)
            else if itkWriteQCTBytesAfterSwappingMacro(float, FLOAT)
            else if itkWriteQCTBytesAfterSwappingMacro(double, DOUBLE)
        }
    }
} // namespace itk
#endif
